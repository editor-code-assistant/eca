(ns eca.llm-api-test
  (:require
   ...
   [eca.llm-providers.openai-codex :as llm-providers.openai-codex]
...
(deftest prompt-selects-codex-profile-only-for-openai-oauth-test
  (let [base-opts {:model "gpt-5.6-sol"
                   ...
                   :tools []
                   :sync? false}]
    (testing "OpenAI OAuth selects the Codex profile and forwards model metadata"
      (let [captured* (atom nil)
            turn-context (llm-providers.openai-codex/new-turn-context "chat-1")]
        (with-redefs [llm-providers.openai/create-response!
                      (fn [opts _callbacks] (reset! captured* opts) :ok)]
          (#'eca.llm-api/prompt!
           (merge base-opts
                  {:provider "openai"
                   ...
                   :config {:providers {"openai" {:api "openai-responses"
                                                  ...
                                                  :models {}}}}})))
        (is (= llm-providers.openai-codex/request-profile
               (:request-profile @captured*)))
        (is (identical? turn-context (:turn-context @captured*)))
        (is (true? (:responses-lite? @captured*)))
        (is (= "low" (:default-reasoning-effort @captured*)))))

    (testing "known GPT-5.6 models use Lite before OAuth model discovery completes"
      (let [captured* (atom nil)]
        (with-redefs [llm-providers.openai/create-response!
                      (fn [opts _callbacks] (reset! captured* opts) :ok)]
          (#'eca.llm-api/prompt!
           (merge base-opts
                  {:provider "openai"
                   :model-capabilities (dissoc (:model-capabilities base-opts)
                                               :codex-responses-lite?
                                               :default-reasoning-effort)
                   :provider-auth {:api-key "oauth-token" :type :auth/oauth}
                   :config {:providers {"openai" {:api "openai-responses"
                                                  :url "https://api.openai.com"
                                                  :models {}}}}})))
        (is (true? (:responses-lite? @captured*)))
    ...
    (testing "OpenAI API keys keep the standard Responses profile"
      (let [captured* (atom nil)]
        (with-redefs [llm-providers.openai/create-response!
                      (fn [opts _callbacks] (reset! captured* opts) :ok)]
          (#'eca.llm-api/prompt!
           (merge base-opts
                  {:provider "openai"
                   ...
                   :config {:providers {"openai" {:api "openai-responses"
                                                  :url "https://api.openai.com"
                                                  :models {}}}}})))
        (is (= :openai-responses (:request-profile @captured*)))))

    (testing "Copilot and custom Responses providers receive no Codex profile"
      (doseq [[provider provider-auth provider-config]
        ...
        (let [captured* (atom nil)]
          ...
          (is (nil? (:request-profile @captured*))
      ...
      (with-redefs [http/post
        ...
        (is (= ["state-1" "state-1"]
               (mapv #(get-in % [:headers "x-codex-turn-state"])
                     (rest @requests*))))
        (is (every? #(= "chat-123" (get-in % [:headers "Session-ID"]))
                    @requests*))))))
...
(deftest openai-retry-reuses-turn-context-test
  (testing "outer retries keep one chat-scoped Codex turn context"
    (let [contexts* (atom [])
          attempts* (atom 0)]
      (with-redefs [eca.llm-api/prompt!
                    (fn [{:keys [turn-context]}]
                      (swap! contexts* conj turn-context)
                      (if (= 1 (swap! attempts* inc))
                        {:error {:status 503
                                 :body "server_is_overloaded"
                                 :message "OpenAI response status: 503"}}
                        {:output-text "ok"
        ...
        (llm-api/sync-or-async-prompt!
         (make-prompt-opts
          {:stream false
           ...
           :on-message-received identity})))
      (is (= 2 (count @contexts*)))
      (is (identical? (first @contexts*) (second @contexts*)))
      (is (= "chat-123" (:session-id (first @contexts*))))
      (is (= "chat-123" (:thread-id (first @contexts*)))))))
