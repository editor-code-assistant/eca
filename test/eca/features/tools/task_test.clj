(ns eca.features.tools.task-test
  (:require
   [clojure.test :refer [deftest is testing]]
   [matcher-combinators.test :refer [match?]]
   [eca.features.tools :as f.tools]
   [eca.features.tools.task :as task]))
...
(deftest chat-isolation-test
  (testing "each chat has its own task list"
    (let [db* (atom {})
          handler (get-in task/definitions ["task" :handler])]
      ...
      (is (= ["Chat 1 Task"] (map :subject (:tasks (task/get-task @db* "chat-1")))))
      (is (= ["Chat 2 Task"] (map :subject (:tasks (task/get-task @db* "chat-2")))))))

  (testing "empty chat returns empty task list"
    (is (= {:next-id 1 :active-summary nil :tasks []}
           (task/get-task {} "nonexistent-chat")))))

;; --- State Access Tests ---

(deftest get-task-test
  (testing "returns empty task list for missing chat state"
    (is (= {:next-id 1 :active-summary nil :tasks []}
           (task/get-task {} "missing-chat"))))

  (testing "returns stored task list state as-is"
    (let [state {:next-id 2
                 :active-summary "Doing stuff"
                 :tasks [{:id 1 :subject "T1" :description "D1" :status :pending :priority :medium :blocked-by #{}}]}
          db {:chats {"c1" {:task state}}}]
      (is (= state (task/get-task db "c1"))))))

;; --- Read Operation Tests ---

(deftest op-read-test
  (let [handler (get-in task/definitions ["task" :handler])]
    (testing "reads empty state"
      (let [result (handler {"op" "read"} {:db {} :chat-id "c1"})]
        (is (not (:error result)))
        (is (match? {:details {:type :task :tasks []}} result))))

    (testing "includes structured details"
      (let [db {:chats {"c1" {:task {:active-summary "My active task"
                                     :tasks [{:id 1
                                              :subject "Task 1"
                                              :description "Desc 1"
                                              :status :in-progress
...
(deftest op-plan-test
  (let [handler (get-in task/definitions ["task" :handler])]
    (testing "creates task list with tasks"
      (let [db* (atom {})
            result (handler {"op" "plan"
                            ...
                            {:db* db* :chat-id "c1"})]
        (is (not (:error result)))
        (is (= 2 (count (get-in @db* [:chats "c1" :task :tasks]))))
        (is (= ["Task 1" "Task 2"] (map :subject (get-in @db* [:chats "c1" :task :tasks]))))
        (is (= :pending (get-in @db* [:chats "c1" :task :tasks 0 :status])))
        (is (= :medium (get-in @db* [:chats "c1" :task :tasks 0 :priority])))))

    (testing "replaces existing task list completely"
      (let [db* (atom {:chats {"c1" {:task {:active-summary "Old summary"
        ...
        (handler {"op" "plan"
                 ...
                 {:db* db* :chat-id "c1"})
        (is (nil? (get-in @db* [:chats "c1" :task :active-summary])))
        (is (not-any? #(= "Old Task" (:subject %))
                      (get-in @db* [:chats "c1" :task :tasks])))
        (is (= 1 (count (get-in @db* [:chats "c1" :task :tasks]))))
        (is (= "New Task" (get-in @db* [:chats "c1" :task :tasks 0 :subject])))
        (is (= 2 (get-in @db* [:chats "c1" :task :next-id])))))
...
(deftest op-update-test
  (let [handler (get-in task/definitions ["task" :handler])]
    (testing "updates task metadata"
      (let [db* (atom {:chats {"c1" {:task {:tasks [{:id 1 :subject "Old" :description "Old D" :status :pending :priority :medium :blocked-by #{}}]}}}})]
        (handler {"op" "update" "id" 1 "task" {"subject" "New" "priority" "high"}} {:db* db* :chat-id "c1"})
        (let [task (first (get-in @db* [:chats "c1" :task :tasks]))]
          (is (= "New" (:subject task)))
          (is (= :high (:priority task))))))

    (testing "rejects status changes via update"
      (let [db* (atom {:chats {"c1" {:task {:tasks [{:id 1 :subject "T1" :description "D1" :status :pending :priority :medium :blocked-by #{}}]}}}})
    ...
    (testing "rejects empty updates"
      (let [db* (atom {:chats {"c1" {:task {:tasks [{:id 1 :subject "T1" :description "D1" :status :pending}]}}}})
  ...
  (let [handler (get-in task/definitions ["task" :handler])]
    (testing "starts pending tasks by ids"
      (let [db* (atom {:chats {"c1" {:task {:tasks [{:id 1 :subject "T1" :description "D1" :status :pending :priority :medium :blocked-by #{}}
        ...
        (is (= :in-progress (get-in @db* [:chats "c1" :task :tasks 0 :status])))
        (is (= :in-progress (get-in @db* [:chats "c1" :task :tasks 1 :status])))
        (is (= "Doing T1 and T2" (get-in @db* [:chats "c1" :task :active-summary])))))

    (testing "does not demote other in-progress tasks"
      (let [db* (atom {:chats {"c1" {:task {:active-summary "Doing T1" :tasks [{:id 1 :subject "T1" :description "D1" :status :in-progress :priority :medium :blocked-by #{}}
                                                    {:id 2 :subject "T2" :description "D2" :status :pending :priority :medium :blocked-by #{}}]}}}})]
    ...
    (testing "rejects starting a done task"
      (let [db* (atom {:chats {"c1" {:task {:tasks [{:id 1 :subject "T1" :description "D1" :status :done :priority :medium :blocked-by #{}}]}}}})
    ...
    (testing "rejects starting a blocked task"
      (let [db* (atom {:chats {"c1" {:task {:tasks [{:id 1 :subject "T1" :description "D1" :status :pending :priority :medium :blocked-by #{}}
        ...
        (is (= :pending (get-in @db* [:chats "c1" :task :tasks 1 :status])))))
...
(deftest op-complete-test
  (let [handler (get-in task/definitions ["task" :handler])]
    (testing "completes tasks by ids"
      (let [db* (atom {:chats {"c1" {:task {:active-summary "My task" :tasks [{:id 1 :subject "T1" :description "D1" :status :in-progress :priority :medium :blocked-by #{}}
                                                    {:id 2 :subject "T2" :description "D2" :status :in-progress :priority :medium :blocked-by #{}}]}}}})]
        (handler {"op" "complete" "ids" [1 2]} {:db* db* :chat-id "c1"})
        (is (= :done (get-in @db* [:chats "c1" :task :tasks 0 :status])))
        (is (= :done (get-in @db* [:chats "c1" :task :tasks 1 :status])))
        (is (nil? (get-in @db* [:chats "c1" :task :active-summary])))))
    ...
    (testing "leaves active summary if tasks are in-progress"
      (let [db* (atom {:chats {"c1" {:task {:active-summary "My task" :tasks [{:id 1 :subject "T1" :description "D1" :status :in-progress :priority :medium :blocked-by #{}}
        ...
        (is (= :done (get-in @db* [:chats "c1" :task :tasks 0 :status])))
        (is (= :in-progress (get-in @db* [:chats "c1" :task :tasks 1 :status])))
        (is (= "My task" (get-in @db* [:chats "c1" :task :active-summary])))))

    (testing "shows unblocked tasks"
      (let [db* (atom {:chats {"c1" {:task {:tasks [{:id 1 :subject "T1" :description "D1" :status :in-progress :priority :medium :blocked-by #{}}
                                                    {:id 2 :subject "T2" :description "D2" :status :pending :priority :medium :blocked-by #{1}}]}}}})
  ...
  (let [handler (get-in task/definitions ["task" :handler])]
    (testing "deletes tasks by ids and clears blocked_by references"
      (let [db* (atom {:chats {"c1" {:task {:tasks [{:id 1 :subject "T1" :description "D1" :status :pending :priority :medium :blocked-by #{}}
        ...
        (is (= 1 (count (get-in @db* [:chats "c1" :task :tasks]))))
        (is (= 3 (get-in @db* [:chats "c1" :task :tasks 0 :id])))
        (is (empty? (get-in @db* [:chats "c1" :task :tasks 0 :blocked-by])))))
...
(deftest op-clear-test
  (let [handler (get-in task/definitions ["task" :handler])]
    (testing "resets to empty state"
      (let [db* (atom {:chats {"c1" {:task {:active-summary "Summary" :tasks [{:id 1}] :next-id 2}}}})]
        (handler {"op" "clear"} {:db* db* :chat-id "c1"})
        (is (empty? (get-in @db* [:chats "c1" :task :tasks])))
        (is (= 1 (get-in @db* [:chats "c1" :task :next-id])))
        (is (nil? (get-in @db* [:chats "c1" :task :active-summary])))))))
...
(deftest blocked-by-validation-test
  (let [handler (get-in task/definitions ["task" :handler])]
    (testing "rejects non-array blocked_by"
      (let [db* (atom {:chats {"c1" {:task {:tasks []}}}})
...
(deftest dependency-cycle-detection-test
  (let [handler (get-in task/definitions ["task" :handler])]
    ...
    (testing "detects cycle involving existing + new tasks"
      (let [db* (atom {:chats {"c1" {:task {:next-id 2
                                            :tasks [{:id 1 :subject "T1" :description "D1" :status :pending :priority :medium :blocked-by #{}}]}}}})
    ...
    (testing "allows forward references in batch add"
      (let [db* (atom {})]
        (handler {"op" "plan"
                 ...
                 {:db* db* :chat-id "c1"})
        (is (= 2 (count (get-in @db* [:chats "c1" :task :tasks]))))
        (is (= #{2} (get-in @db* [:chats "c1" :task :tasks 0 :blocked-by])))))
    ...
    (testing "allows reference to earlier task in batch"
      (let [db* (atom {})]
        (handler {"op" "plan"
                 ...
                 {:db* db* :chat-id "c1"})
        (is (= #{1} (get-in @db* [:chats "c1" :task :tasks 1 :blocked-by])))))
...
(deftest task-tool-call-details-test
  (testing "task details are propagated to tool call details for clients"
    (let [result {:error false
                  :details {:type :task
                            :tasks []
                            :summary {:done 0 :in-progress 0 :pending 0 :total 0}}
                  :contents [{:type :text :text "Task list created"}]}
